...
import com.puffer.util.lang.CaseFormatUtil;
...
public enum CaseFormat {
    ...
    UPPER_SPACE_SPEPARATOR(CharMatcher.is(' '), " ") {
        @Override
        String normalizeWord(String word) {
            return firstCharOnlyToUpper(word);
//      return Ascii.toUpperCase(word);
    ...
    UPPER_SPACE(CharMatcher.inRange('A', 'Z'), " ") {
        @Override
        String normalizeWord(String word) {
            return firstCharOnlyToUpper2(word, false);
        }
    },
    /**
     * 排除介词
     */
    UPPER_SPACE_PREP(CharMatcher.inRange('A', 'Z'), " ") {
        @Override
        String normalizeWord(String word) {
            return firstCharOnlyToUpper2(word, true);
        }
    ...
    private static String firstCharOnlyToUpper2(String word, boolean isPrep) {
        if (word.isEmpty()) {
            return word;
        }

        if (isPrep && matchPrep(word)) {
            return Ascii.toLowerCase(word);
        }
    ...
    private static boolean matchPrep(String word) {
        return CaseFormatUtil.PREP_LIST.contains(Ascii.toLowerCase(word));
    }
}
